...
courseRouter.post(
    ...
    async (request:Request, response: Response) => {
        try {
            ...
            const newCourse = await courseController.createCourse(request.body.authorId, request.body.categoryId, course);
...
coursesRouter.get('/:categoryId',  async (request:Request, response:Response) => {
    try {
        ...
        return response.status(200).json(await courseController.getCoursesByCategory(categoryId));
    } catch (error: any) {
...
courseRouter.put('/:id',  
    ...
    async (request: Request, response: Response) => {
        try {
            const course = Course.parse( request.body);
            const updatedCourse = await courseController.updateCourse(course, parseInt(request.params.id));
