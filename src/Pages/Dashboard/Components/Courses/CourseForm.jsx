...
function CourseForm(props) {
	...
	return (
		<form
			...
			className='flex flex-col gap-3 w-full mt-9 text-[#6b7280] font-normal'>
			...
			<div className='flex flex-col w-full gap-2'>
				...
				<textarea
					...
					onChange={handleChange}
					className='border-2 border-[#6b7280] border-solid outline-none bg-transparent py-3 px-2 hover:border-[#5624d0] focus:border-[#5624d0] min-h-[100px]'
					required></textarea>
			</div>
			<div className='flex flex-col w-full gap-2'>
			...
			<div className='flex flex-col w-full gap-2'>
				...
				<select
					...
					<option
						key={"rnkdmi290snfjs"}
						className='bg-[#1b1f23] text-[#6b7280] '
						value=''>
						Select a category
					</option>
					{categories &&
						categories?.map((category) => (
							<option
								key={category.id}
								className='bg-[#1b1f23] text-[#6b7280] border-solid border-[#6b7280] hover:bg-[#5624d0] hover:text-[#1b1f23] mt-2'
								value={category?.id}>
								{category.title}
							</option>
						))}
